...
@app.route("/setNewsEmbedding", methods=["POST", "GET"])
def set_news_embedding():
    # 不传参数时，自动生成所有未生成向量的新闻
    generate_news_embedding(titles_obj=None)
    return success()


@app.route("/setUserRecommenderList", methods=["POST", "GET"])
def set_user_recommender_list():
    # 不传参数时，自动更新所有用户的推荐列表
    generate_user_recommender_list(user_info=None)
    return success()
